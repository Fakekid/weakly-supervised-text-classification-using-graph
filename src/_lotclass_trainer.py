...
class LOTClassTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.pretrained_lm = '/chj/dev/lianxiaolei/bert_pretrain/mlm_model_agnews'
    ...
    def create_dataset(self, dataset_dir, text_file, label_file, loader_name, find_label_name=False,
        ...
        if find_label_name:
            ...
            if os.path.exists(loader_file):
                ...
                chunk_size = ceil(len(docs) / self.num_cpus)
                chunks = [docs[x:x + chunk_size] for x in range(0, len(docs), chunk_size)]
                # 提取文档中的类别指示词（通过label_name_occurrence函数）
    ...
    def label_name_occurrence(self, docs):
        ...
        label_name_idx = []
        for doc in docs:
            # 找到label index、如果word存在于tokenizer中，则进行mask
            # result为(doc, label_idx)
    ...
    # category_vocabulary中向进程池扔的辅助函数
    ...
    def prepare_mcp_dist(self, rank, top_pred_num=50, match_threshold=20, loader_name="mcp_train.pt"):
        ...
        try:
            ...
            for batch in wrap_train_dataset_loader:
                wrap_train_dataset_loader.set_description(f'doc hits: {category_doc_num}')
                with torch.no_grad():
                    input_ids = batch[0].to(rank)
                    input_mask = batch[1].to(rank)
                    ...
                    batch_cate_top_preds = {}
                    batch_docs = {}
                    # 遍历指示词库
                    for i, category_vocab in self.category_vocab.items():
                        cate_top_preds.setdefault(i, [])
                        cate_docs.setdefault(i, [])

                        match_idx = torch.zeros_like(sorted_res).bool()  # 用于统计每个位置的前50个预测token中有多少是当前类别库下的token
                        for word_id in category_vocab:
                            match_idx = (sorted_res == word_id) | match_idx
                        # 统计每个位置的前50个词里有多少个属于当前类别下的词库（注意这里是每个位置，并不是只统计label-indicator位置）
                        match_count = torch.sum(match_idx.int(), dim=-1)
                        valid_idx = (match_count > match_threshold) & (input_mask > 0)

                        # # 将此batch中所有类别指示词位置的top50 logits输出
                        # # print(sorted_res.shape, valid_idx.shape)
                        # batch_cate_top_preds[i].append(sorted_res[valid_idx])
                        # batch_docs[i].append(input_ids)

                        valid_doc = torch.sum(valid_idx, dim=-1) > 0
                        # 只要有符合条件的token，就在所有命中类别词的token位置设置类别ID，不符合条件的就是-1
                        if valid_doc.any():
                            ...
                            # 将此batch中所有类别指示词位置的top50 logits输出
                            # print(sorted_res.shape, input_ids.shape)
                            # print(valid_idx.shape, torch.sum(valid_idx.int(), dim=-1).shape)
                            # print(sorted_res[valid_idx].shape, input_ids[torch.sum(valid_idx.int(), dim=-1) > 0].shape)

                            cate_top_preds[i].append(sorted_res[valid_idx].cpu().numpy())
                            cate_docs[i].append(input_ids[torch.sum(valid_idx.int(), dim=-1) > 0].cpu().numpy())

            with open('cate_top_preds.che', 'wb') as fout:
                pkl.dump(cate_top_preds, fout)
            with open('cate_docs.che', 'wb') as fout:
                pkl.dump(cate_docs, fout)
            print('Cached done.')
